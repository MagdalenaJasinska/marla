...
?><article 
	...
	trx_addons_get_template_part('templates/tpl.featured.php',
								...
								apply_filters('trx_addons_filter_args_featured', array(
												'post_info' => '',
												'thumb_size' => trx_addons_get_thumb_size($number<=$featured ? 'medium' : 'magazine')
	...
	if ( !in_array($post_format, array('link', 'aside', 'status', 'quote')) ) {
		...
		<div class="post_header entry-header">
			...
			if ( in_array( get_post_type(), array( 'post', 'attachment' ) ) ) {
				...
				?><span class="post_date"><a href="<?php echo esc_url(get_permalink()); ?>"><?php echo get_the_date(); ?></a></span><span class="post_author"><?php esc_html_e('by ', 'avicenna');?><span><?php the_author_link(); ?></span></span></div><?php
			}
			?>
		</div><!-- .entry-header -->
		<?php
