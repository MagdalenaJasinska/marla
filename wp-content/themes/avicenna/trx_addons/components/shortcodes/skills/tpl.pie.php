...
wp_enqueue_script( 'trx-addons-chart', trx_addons_get_file_url(TRX_ADDONS_PLUGIN_SHORTCODES . 'skills/chart.min.js'), array('jquery'), null, true );
		...
		$data .= ($args['columns'] > 0 ? '<div class="sc_skills_column '.esc_attr(trx_addons_get_column_class(1, $args['columns'])).'">' : '')
				. '<div class="sc_skills_item_wrap">'
					...
					. (!empty($v['title']) 
									...
									. (!empty($icon) 
										? (!empty($v['icon_type']) && $v['icon_type'] == 'sow'
											...
											: '<span class="sc_skills_icon '.esc_attr($icon).'">'
												. (!empty($img)
													? '<img class="sc_icon_as_image" src="'.esc_url($img).'" >'
													: '')
												. '</span>'
											)
										: '') 
							...
							: '')
                    . (!empty($v['descr'])
                        ? '<div class="sc_skills_item_descr">'
                        . nl2br(str_replace('|', "\n", esc_html($v['descr'])))
                        . '</div>'
                        : '')
				. '</div>'
