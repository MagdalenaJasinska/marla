<div class="front_page_section front_page_section_testimonials<?php
			$avicenna_scheme = avicenna_get_theme_option('front_page_testimonials_scheme');
			if (!avicenna_is_inherit($avicenna_scheme)) echo ' scheme_'.esc_attr($avicenna_scheme);
			echo ' front_page_section_paddings_'.esc_attr(avicenna_get_theme_option('front_page_testimonials_paddings'));
		?>"<?php
		$avicenna_css = '';
		$avicenna_bg_image = avicenna_get_theme_option('front_page_testimonials_bg_image');
...
?>><?php
	// Add anchor
	$avicenna_anchor_icon = avicenna_get_theme_option('front_page_testimonials_anchor_icon');	
	$avicenna_anchor_text = avicenna_get_theme_option('front_page_testimonials_anchor_text');	
	...
	<div class="front_page_section_inner front_page_section_testimonials_inner<?php
			if (avicenna_get_theme_option('front_page_testimonials_fullheight'))
				echo ' avicenna-full-height sc_layouts_flex sc_layouts_columns_middle';
			?>"<?php
			$avicenna_css = '';
			$avicenna_bg_mask = avicenna_get_theme_option('front_page_testimonials_bg_mask');
			$avicenna_bg_color = avicenna_get_theme_option('front_page_testimonials_bg_color');
	...
	?>>
		<div class="front_page_section_content_wrap front_page_section_testimonials_content_wrap content_wrap">
			<?php
			// Caption
			$avicenna_caption = avicenna_get_theme_option('front_page_testimonials_caption');
			if (!empty($avicenna_caption) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><h2 class="front_page_section_caption front_page_section_testimonials_caption front_page_block_<?php echo !empty($avicenna_caption) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_post($avicenna_caption); ?></h2><?php
			}
		
			// Description (text)
			$avicenna_description = avicenna_get_theme_option('front_page_testimonials_description');
			if (!empty($avicenna_description) || (current_user_can('edit_theme_options') && is_customize_preview())) {
				?><div class="front_page_section_description front_page_section_testimonials_description front_page_block_<?php echo !empty($avicenna_description) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_post(wpautop($avicenna_description)); ?></div><?php
			}
			...
			?><div class="front_page_section_output front_page_section_testimonials_output"><?php 
				if (is_active_sidebar('front_page_testimonials_widgets')) {
					...
					if (!avicenna_exists_trx_addons())
						...
						avicenna_customizer_need_widgets_message('front_page_testimonials_caption', 'ThemeREX Addons - Testimonials');
				}
			?></div>
		</div>
	</div>
