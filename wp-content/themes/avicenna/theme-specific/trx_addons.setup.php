...
------------------------------------------------------------------------------- */
if (!function_exists('avicenna_trx_addons_theme_specific_setup1')) {
	add_filter( 'trx_addons_filter_components_editor', 'avicenna_trx_addons_theme_specific_components');
	function avicenna_trx_addons_theme_specific_components($enable=false) {
		return AVICENNA_THEME_FREE
					? false		// Free version
...
if (!function_exists('avicenna_trx_addons_theme_specific_setup1')) {
	add_action( 'after_setup_theme', 'avicenna_trx_addons_theme_specific_setup1', 1 );
	add_action( 'trx_addons_action_save_options', 'avicenna_trx_addons_theme_specific_setup1', 1 );
	function avicenna_trx_addons_theme_specific_setup1() {
		if (avicenna_exists_trx_addons()) {
			add_filter( 'trx_addons_cv_enable',					'avicenna_trx_addons_cv_enable');
			add_filter( 'trx_addons_demo_enable',				'avicenna_trx_addons_demo_enable');
			add_filter( 'trx_addons_filter_edd_themes_market',	'avicenna_trx_addons_edd_themes_market_enable');
			add_filter( 'trx_addons_cpt_list',					'avicenna_trx_addons_cpt_list');
			add_filter( 'trx_addons_sc_list',					'avicenna_trx_addons_sc_list');
			add_filter( 'trx_addons_widgets_list',				'avicenna_trx_addons_widgets_list');
		}
	}
}

// CV
if ( !function_exists( 'avicenna_trx_addons_cv_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_cv_enable', 'avicenna_trx_addons_cv_enable');
	function avicenna_trx_addons_cv_enable($enable=false) {
...
// Demo mode
if ( !function_exists( 'avicenna_trx_addons_demo_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_demo_enable', 'avicenna_trx_addons_demo_enable');
	function avicenna_trx_addons_demo_enable($enable=false) {
...
// EDD Themes market
if ( !function_exists( 'avicenna_trx_addons_edd_themes_market_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_edd_themes_market', 'avicenna_trx_addons_edd_themes_market_enable')) {
	function avicenna_trx_addons_edd_themes_market_enable($enable=false) {
...
// API
if ( !function_exists( 'avicenna_trx_addons_api_list' ) ) {
	//Handler of the add_filter('trx_addons_api_list',	'avicenna_trx_addons_api_list');
	function avicenna_trx_addons_api_list($list=array()) {
...
// CPT
if ( !function_exists( 'avicenna_trx_addons_cpt_list' ) ) {
	//Handler of the add_filter('trx_addons_cpt_list',	'avicenna_trx_addons_cpt_list');
	function avicenna_trx_addons_cpt_list($list=array()) {
...
// Shortcodes
if ( !function_exists( 'avicenna_trx_addons_sc_list' ) ) {
	//Handler of the add_filter('trx_addons_sc_list',	'avicenna_trx_addons_sc_list');
	function avicenna_trx_addons_sc_list($list=array()) {
...
// Widgets
if ( !function_exists( 'avicenna_trx_addons_widgets_list' ) ) {
	//Handler of the add_filter('trx_addons_widgets_list',	'avicenna_trx_addons_widgets_list');
	function avicenna_trx_addons_widgets_list($list=array()) {
...
if ( !function_exists( 'avicenna_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'avicenna_trx_addons_menu_cache');
	function avicenna_trx_addons_menu_cache($list=array()) {
...
if (!function_exists('avicenna_trx_addons_localize_script')) {
	...
	function avicenna_trx_addons_localize_script($arr) {
		$arr['alter_link_color'] = avicenna_get_scheme_color('alter_link');
		return $arr;
	}
}
...
if ( !function_exists( 'avicenna_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'avicenna_trx_addons_sc_type', 10, 2);
	function avicenna_trx_addons_sc_type($list, $sc) {
...
if ( !function_exists( 'avicenna_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'avicenna_trx_addons_sc_atts', 10, 2);
	function avicenna_trx_addons_sc_atts($atts, $sc) {
...
if ( !function_exists( 'avicenna_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'avicenna_trx_addons_sc_map', 10, 2);
	function avicenna_trx_addons_sc_map($params, $sc) {
		...
		$param = array(
			...
			'heading' => esc_html__('Color style', 'avicenna'),
			'description' => wp_kses_data( __('Select color style to decorate this block', 'avicenna') ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label' => true,
			'value' => array_flip(avicenna_get_list_sc_color_styles()),
			'type' => 'dropdown'
		);
...
if ( !function_exists( 'avicenna_trx_addons_sow_map' ) ) {
	add_filter( 'trx_addons_sow_map', 'avicenna_trx_addons_sow_map', 10, 2);
	function avicenna_trx_addons_sow_map($params, $sc) {

		// Param 'color_style'
		$param = array(
			'color_style' => array(
				'label' => esc_html__('Color style', 'avicenna'),
				'description' => wp_kses_data( __('Select color style to decorate this block', 'avicenna') ),
				'options' => avicenna_get_list_sc_color_styles(),
				'default' => 'default',
				'type' => 'select'
			)
		);
		if (in_array($sc, array('trx_sc_button')))
			avicenna_array_insert_after($params, 'size', $param);
		else if (in_array($sc, array('trx_sc_action', 'trx_sc_blogger', 'trx_sc_cars', 'trx_sc_courses', 'trx_sc_content', 'trx_sc_dishes',
								'trx_sc_events', 'trx_sc_form',	'trx_sc_googlemap', 'trx_sc_portfolio', 'trx_sc_price', 'trx_sc_promo',
								'trx_sc_properties', 'trx_sc_services', 'trx_sc_team', 'trx_sc_testimonials', 'trx_sc_title',
								'trx_widget_audio', 'trx_widget_twitter')))
			avicenna_array_insert_after($params, 'title_align', $param);
		return $params;
	}
}

// Add classes to the shortcode's output
if ( !function_exists( 'avicenna_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'avicenna_trx_addons_sc_output', 10, 4);
	function avicenna_trx_addons_sc_output($output, $sc, $atts, $content) {
		...
		if (in_array($sc, array('trx_sc_action'))) {
			...
			if (!empty($atts['color_style']) && !avicenna_is_inherit($atts['color_style']))
...
if ( !function_exists( 'avicenna_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'avicenna_trx_addons_sc_item_title_tag');
	function avicenna_trx_addons_sc_item_title_tag($tag='') {
		return $tag=='h1' ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( !function_exists( 'avicenna_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'avicenna_trx_addons_sc_item_button_args', 10, 3);
	function avicenna_trx_addons_sc_item_button_args($args, $sc, $sc_args) {
...
if ( !function_exists( 'avicenna_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title',	'avicenna_trx_addons_slider_title', 10, 2 );
	function avicenna_trx_addons_slider_title($title, $data) {
		$title = '';
		if (!empty($data['title'])) 
...
if ( !function_exists( 'avicenna_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles',	'avicenna_trx_addons_sc_googlemap_styles');
	function avicenna_trx_addons_sc_googlemap_styles($list) {
		$list['dark'] = esc_html__('Dark', 'avicenna');
		return $list;
	}
}
...
if ( !function_exists( 'avicenna_trx_addons_tiny_mce_style_formats' ) ) {
	add_filter( 'trx_addons_filter_tiny_mce_style_formats', 'avicenna_trx_addons_tiny_mce_style_formats');
	function avicenna_trx_addons_tiny_mce_style_formats($style_formats) {
		...
		if (is_array($style_formats) && count($style_formats)>0 ) {
			foreach ($style_formats as $k=>$v) {
                if ( $v['title'] == esc_html__('List styles', 'avicenna') ) {
                    $style_formats[$k]['items'][] = array(
                        'title' => esc_html__('Custom', 'avicenna'),
                        'selector' => 'ol',
                        'classes' => 'trx_addons_list trx_addons_list_custom'
                    );
                }
                if ( $v['title'] == esc_html__('List styles', 'avicenna') ) {
                    $style_formats[$k]['items'][] = array(
                        'title' => esc_html__('Leaf', 'avicenna'),
                        'selector' => 'ul',
                        'classes' => 'trx_addons_list_leaf'
                    );
                }
                if ( $v['title'] == esc_html__('Inline', 'avicenna') ) {
                    $style_formats[$k]['items'][] = array(
                        'title' => esc_html__('Hovered text', 'avicenna'),
                        'inline' => 'span',
                        'classes' => 'trx_addons_hovered'
                    );
                }
			}
		}
...
if ( !function_exists( 'avicenna_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'avicenna_filter_allow_override_header_image', 'avicenna_trx_addons_allow_override_header_image' );
	function avicenna_trx_addons_allow_override_header_image($allow) {
		return avicenna_is_team_page() ? false : $allow;
	}
}

// Get thumb size for the team items
if ( !function_exists( 'avicenna_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size',	'avicenna_trx_addons_thumb_size', 10, 2);
	function avicenna_trx_addons_thumb_size($thumb_size='', $type='') {
		if ($type == 'team-default')
			$thumb_size = avicenna_get_thumb_size('big');
		if ($type == 'services-hover')
			$thumb_size = avicenna_get_thumb_size('services');
		if ($type == 'services-light')
			$thumb_size = avicenna_get_thumb_size('light');
		return $thumb_size;
	}
}

if ( !function_exists( 'avicenna_trx_addons_filter_sc_item_link_classes' ) ) {
	add_filter( 'trx_addons_filter_sc_item_link_classes',	'avicenna_trx_addons_filter_sc_item_link_classes', 10, 3);
	function avicenna_trx_addons_filter_sc_item_link_classes($clases='', $type='', $args='') {
            if ($type == 'sc_form') {
                if(isset($args['color_style']) && $args['color_style'] == 'dark') {
                    $clases = 'sc_button sc_button_bordered sc_button_size_small';
                }
            }
            if ($type == 'sc_services') {
                $clases = 'sc_button sc_button_size_small';
            }
		return $clases;
	}
}

if ( !function_exists( 'avicenna_trx_addons_sc_param_group_fields' ) ) {
	add_filter( 'trx_addons_sc_param_group_fields',	'avicenna_trx_addons_sc_param_group_fields', 10, 2);
	function avicenna_trx_addons_sc_param_group_fields($params='', $type='') {
		if ($type == 'trx_sc_skills')
            $params = array_merge($params, array(
                'descr' => array(
                    'label' => esc_html__('Description', 'avicenna'),
                    'description' => esc_html__( 'Enter description of the item (only for pie type)', 'avicenna' ),
                    'type' => 'text'
                ),
            ));
		return $params;
...
if ( !function_exists( 'avicenna_trx_addons_sc_param_group_params' ) ) {
	add_filter( 'trx_addons_sc_param_group_params',	'avicenna_trx_addons_sc_param_group_params', 10, 2);
	function avicenna_trx_addons_sc_param_group_params($params='', $type='') {
		if ($type == 'trx_sc_skills')
            $params = array_merge($params, array(array(
                'param_name' => 'descr',
                'heading' => esc_html__( 'Description', 'avicenna' ),
                'description' => esc_html__( 'Enter description of this item (only for pie type)', 'avicenna' ),
                'admin_label' => true,
                'edit_field_class' => 'vc_col-sm-4',
                'type' => 'textarea',
            )));
...
if (!function_exists('avicenna_trx_addons_override_options_fields')) {
	add_filter( 'trx_addons_filter_meta_box_fields', 'avicenna_trx_addons_override_options_fields', 10, 2);
	function avicenna_trx_addons_override_options_fields($mb, $post_type) {
		if (defined('TRX_ADDONS_CPT_TEAM_PT') && $post_type==TRX_ADDONS_CPT_TEAM_PT) {
			$mb['email'] = array(
				"title" => esc_html__("E-mail",  'avicenna'),
				"desc" => wp_kses_data( __("Team member's email", 'avicenna') ),
				"std" => "",
				"details" => true,
				"type" => "text"
			);
