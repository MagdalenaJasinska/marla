...
if (!function_exists('avicenna_customizer_get_css')) {

	function avicenna_customizer_get_css($colors=null, $fonts=null, $remove_spaces=true, $only_scheme='') {
		...
		if ($fonts) {
			...
			$fonts = avicenna_customizer_add_theme_fonts($fonts);
...
EOT;
			$rez = apply_filters('avicenna_filter_get_css', $rez, false, $fonts, '');
			$css['fonts'] = $rez['fonts'];
...
EOT;
		...
		//--------------------------------------
		if ($colors !== false) {
			$schemes = empty($only_scheme) ? array_keys(avicenna_get_list_schemes()) : array($only_scheme);
	
			if (count($schemes) > 0) {
				$rez = array();
				foreach ($schemes as $scheme) {
					// Prepare colors
					if (empty($only_scheme)) $colors = avicenna_get_scheme_colors($scheme);
...
.avicenna_tabs .avicenna_tabs_titles li a:hover {
	color: {$colors['inverse_link']};
	background-color: {$colors['text_link']};
}
div.esg-filter-wrapper .esg-filterbutton.selected > span,
.mptt-navigation-tabs li.active a,
.avicenna_tabs .avicenna_tabs_titles li.ui-state-active a {
	color: {$colors['bg_color']};
	background-color: {$colors['text_dark']};
}
...
.strong-rating input:checked + label:before, .error .strong-rating input:checked + label:before,
...
.header_position_over .top_panel .sc_layouts_iconed_text .sc_layouts_item_details_line1,
.header_position_over .top_panel .sc_layouts_iconed_text .sc_layouts_item_details_line2 {
    color: {$colors['bg_color']};
}
.widget_rss .widget_title a {
    color: {$colors['bg_color']};
}
.header_position_over .top_panel .sc_layouts_row_type_compact input[placeholder]::-webkit-input-placeholder,
.header_position_over .top_panel .sc_layouts_row_type_compact textarea[placeholder]::-webkit-input-placeholder	{ color: {$colors['bg_color']}; }
.header_position_over .top_panel .sc_layouts_row_type_compact input[placeholder]::-moz-placeholder,
.header_position_over .top_panel .sc_layouts_row_type_compact textarea[placeholder]::-moz-placeholder				{ color: {$colors['bg_color']}; }
.header_position_over .top_panel .sc_layouts_row_type_compact input[placeholder]:-ms-input-placeholder,
.header_position_over .top_panel .sc_layouts_row_type_compact textarea[placeholder]:-ms-input-placeholder			{ color: {$colors['bg_color']}; }
.header_position_over .top_panel .sc_layouts_row_type_compact input[placeholder]::placeholder,
.header_position_over .top_panel .sc_layouts_row_type_compact textarea[placeholder]::placeholder					{ color: {$colors['bg_color']}; }
...
EOT;
					...
					$rez = apply_filters('avicenna_filter_get_css', $rez, $colors, false, $scheme);
		...
		return apply_filters( 'avicenna_filter_prepare_css', $css_str, $remove_spaces );
	}
}
?>