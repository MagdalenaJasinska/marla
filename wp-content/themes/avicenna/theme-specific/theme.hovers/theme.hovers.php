...
if (!function_exists('avicenna_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'avicenna_hovers_theme_setup3', 3 );
	function avicenna_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		avicenna_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'avicenna'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'avicenna') ),
				"std" => 'default',
				"options" => array(
					'default'		=> esc_html__('Fade',				'avicenna'),
					'slide_left'	=> esc_html__('Slide from Left',	'avicenna'),
					'slide_right'	=> esc_html__('Slide from Right',	'avicenna'),
					'slide_top'		=> esc_html__('Slide from Top',		'avicenna'),
					'slide_bottom'	=> esc_html__('Slide from Bottom',	'avicenna'),

				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'avicenna'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'avicenna') ),
				"std" => 'dots',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'avicenna'),
					'icon'	=> esc_html__('Icon',	'avicenna'),
					'icons'	=> esc_html__('Icons',	'avicenna'),
					'zoom'	=> esc_html__('Zoom',	'avicenna'),
					'fade'	=> esc_html__('Fade',	'avicenna'),
					'slide'	=> esc_html__('Slide',	'avicenna'),
					'pull'	=> esc_html__('Pull',	'avicenna'),
					'border'=> esc_html__('Border',	'avicenna')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('avicenna_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'avicenna_hovers_theme_setup9', 9 );
	function avicenna_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'avicenna_hovers_frontend_scripts', 1010 );
		add_filter( 'avicenna_filter_localize_script','avicenna_hovers_localize_script' );
		add_filter( 'avicenna_filter_merge_scripts',	'avicenna_hovers_merge_scripts' );
		add_filter( 'avicenna_filter_merge_styles',	'avicenna_hovers_merge_styles' );
		add_filter( 'avicenna_filter_get_css', 		'avicenna_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'avicenna_hovers_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'avicenna_hovers_frontend_scripts', 1010 );
	function avicenna_hovers_frontend_scripts() {
		if ( avicenna_is_on(avicenna_get_theme_option('debug_mode')) && avicenna_get_file_dir('theme-specific/theme.hovers/theme.hovers.js')!='' )
			wp_enqueue_script( 'avicenna-hovers', avicenna_get_file_url('theme-specific/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( avicenna_is_on(avicenna_get_theme_option('debug_mode')) && avicenna_get_file_dir('theme-specific/theme.hovers/theme.hovers.css')!='' )
			wp_enqueue_style( 'avicenna-hovers',  avicenna_get_file_url('theme-specific/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('avicenna_hovers_merge_scripts')) {
	//Handler of the add_filter( 'avicenna_filter_merge_scripts', 'avicenna_hovers_merge_scripts' );
	function avicenna_hovers_merge_scripts($list) {
...
if (!function_exists('avicenna_hovers_merge_styles')) {
	//Handler of the add_filter( 'avicenna_filter_merge_styles', 'avicenna_hovers_merge_styles' );
	function avicenna_hovers_merge_styles($list) {
...
if (!function_exists('avicenna_hovers_localize_script')) {
	//Handler of the add_filter( 'avicenna_filter_localize_script','avicenna_hovers_localize_script' );
	function avicenna_hovers_localize_script($arr) {
		$arr['button_hover'] = avicenna_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('avicenna_hovers_add_icons') ) {
	function avicenna_hovers_add_icons($hover, $args=array()) {
		...
		// Hover style 'Shop'
			...
			<div class="icons">
				...
				if (!is_object($args['cat']) && $product->is_purchasable() && $product->is_in_stock()) {
					avicenna_show_layout(apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? esc_html__('Buy now', 'avicenna') : '')
										. '</a>',
										$product));
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') {
						if (is_object($args['cat']))
							esc_html_e('View products', 'avicenna');
						else
							esc_html_e('Details', 'avicenna');
					}
				?></a>
			</div>
			<?php
...
// Add styles into CSS
if ( !function_exists( 'avicenna_hovers_get_css' ) ) {
	//Handler of the add_filter( 'avicenna_filter_get_css', 'avicenna_hovers_get_css', 10, 4 );
	function avicenna_hovers_get_css($css, $colors, $fonts, $scheme='') {
